...
{
public:
	hnRibbonContextCategory* contextCategory;
    QList<int> tabPageIndex;
    bool operator ==(const hnRibbonContextCategory* contextPage)
    {
...
public:
    ...
    QMargins widgetBord;
	hnRibbonBar* MainClass;
    const int tabBarHight;
    QColor titleTextColor;
    QAbstractButton* applitionButton;
	hnRibbonTabBar* ribbonTabBar;
	hnRibbonStackedWidget* stackedContainerWidget;
    QList<ContextCategoryManagerData> currentShowingContextCategory;
    int iconRightBorderPosition;///< 标题栏x值得最小值，在有图标和快捷启动按钮，此值都需要变化
	hnRibbonControlButton* hidePannelButton;///< 隐藏面板按钮
	hnRibbonButtonGroupWidget* tabBarRightSizeButtonGroupWidget;///< 在tab bar旁边的button group widget
    QColor tabBarBaseLineColor;///< tabbar底部的线条颜色
	hnRibbonQuickAccessBar* quickAccessBar;///< 快速响应栏
	hnRibbonBar::RibbonStyle ribbonStyle;///< ribbon的风格
	hnRibbonBar::RibbonStyle lastShowStyle;///< ribbon的风格
    int unusableTitleRegion;///<不可用的右边标题栏长度，主要是关闭按钮的位置
    int meanBarHeight;
	hnRibbonBar::RibbonMode currentRibbonMode;///< 记录当前模式
	hnRibbonBarPrivate(hnRibbonBar* par)
        :titleBarHight(30)
        ,widgetBord(0,0,0,0)
        ,tabBarHight(30)
        ...
        ,ribbonStyle(hnRibbonBar::OfficeStyle)
        ,lastShowStyle(hnRibbonBar::OfficeStyle)
        ,unusableTitleRegion(100)
        ,meanBarHeight(160)
        ,currentRibbonMode(hnRibbonBar::NormalRibbonMode)
    {
        MainClass = par;
    }
    ...
    void init()
    {
        ...
        applitionButton->setGeometry(widgetBord.left(),10/*titleBarHight+widgetBord.top()*/,56,60);
        MainClass->connect(applitionButton,&QAbstractButton::clicked
                     ,MainClass,&hnRibbonBar::applitionButtonClicked);
        //
        ...
        MainClass->connect(ribbonTabBar,&QTabBar::currentChanged
                     ,MainClass,&hnRibbonBar::onCurrentRibbonTabChanged);
        MainClass->connect(ribbonTabBar,&QTabBar::tabBarClicked
                     ,MainClass,&hnRibbonBar::onCurrentRibbonTabClicked);
        MainClass->connect(ribbonTabBar,&QTabBar::tabBarDoubleClicked
                     ,MainClass,&hnRibbonBar::onCurrentRibbonTabDoubleClicked);
        //
        ...
        stackedContainerWidget->connect(stackedContainerWidget,&hnRibbonStackedWidget::hidWindow
                                        ,MainClass,&hnRibbonBar::onStackWidgetHided);
    ...
    bool isContainContextCategoryInList(hnRibbonContextCategory* contextCategory)
    {
    ...
    void setHideMode()
    {
        currentRibbonMode = hnRibbonBar::MinimumRibbonMode;
...
hnRibbonBar::hnRibbonBar(QWidget *parent):QMenuBar(parent)
  ,m_d(new hnRibbonBarPrivate(this))
{
    m_d->init();
    connect(parent,&QWidget::windowTitleChanged,this,&hnRibbonBar::onWindowTitleChanged);
    connect(parent,&QWidget::windowIconChanged,this,&hnRibbonBar::onWindowIconChanged);
}



QAbstractButton *hnRibbonBar::applitionButton()
{
    return m_d->applitionButton;
}

void hnRibbonBar::setApplitionButton(QAbstractButton *btn)
{
    ...
    if(btn)
    {
        ...
        {
            btn->setObjectName(QStringLiteral("hnRibbonApplitionButton"));
        }
    }
    repaint();
}

hnRibbonTabBar *hnRibbonBar::ribbonTabBar()
{
...
///
hnRibbonCategory *hnRibbonBar::addCategoryPage(const QString &title)
{
    hnRibbonCategory* catagory =  RibbonSubElementDelegate->createRibbonCategory(this);
    ...
    connect(catagory,&QWidget::windowTitleChanged,this,&hnRibbonBar::onCategoryWindowTitleChanged);
    return catagory;
}

hnRibbonContextCategory *hnRibbonBar::addContextCategory(const QString &title, const QColor &color, const QVariant &id)
{
    hnRibbonContextCategory* context = RibbonSubElementDelegate->createRibbonContextCategory(this);
    context->setContextTitle(title);
    context->setId(id);
    context->setContextColor(color);
    connect(context,&hnRibbonContextCategory::categoryPageAdded
            ,this,&hnRibbonBar::onContextsCategoryPageAdded);
    return context;
}
///
/// \brief 显示上下文标签
...
///
void hnRibbonBar::showContextCategory(hnRibbonContextCategory *context)
{
    ...
    {
        hnRibbonCategory* category = context->categoryPage(i);
...
void hnRibbonBar::hideContextCategory(hnRibbonContextCategory *context)
{
...
void hnRibbonBar::setContextCategoryVisible(hnRibbonContextCategory *context, bool visible)
{
    if(visible)
    {
...
/// \param isHide
///
void hnRibbonBar::setHideMode(bool isHide)
{
    if(isHide)
    {
        m_d->setHideMode();
...
/// \return
///
bool hnRibbonBar::isRibbonBarHideMode() const
{
...
///
void hnRibbonBar::showHideModeButton(bool isShow)
{
    if(isShow)
    {
...
/// \return
///
bool hnRibbonBar::isShowHideModeButton() const
{
    return (nullptr != m_d->hidePannelButton);
}

int hnRibbonBar::tabBarHeight() const
{
    return m_d->tabBarHight;
}

int hnRibbonBar::titleBarHeight() const
{
    return m_d->titleBarHight;
}


void hnRibbonBar::onWindowTitleChanged(const QString &title)
{
    Q_UNUSED(title);
    update();
}

void hnRibbonBar::onWindowIconChanged(const QIcon &icon)
{
    if(!icon.isNull())
    {
...
void hnRibbonBar::onCategoryWindowTitleChanged(const QString &title)
{
...
/// \brief ribbon的显示界面隐藏
///
void hnRibbonBar::onStackWidgetHided()
{
...
/// \param index
///
void hnRibbonBar::onCurrentRibbonTabChanged(int index)
{
    QVariant var = m_d->ribbonTabBar->tabData(index);
    hnRibbonCategory* category = nullptr;
    if(var.isValid())
    {
        quint64 p = var.value<quint64>();
        category = (hnRibbonCategory*)p;
    }
    if(category)
    {
...
/// \param index
///
void hnRibbonBar::onCurrentRibbonTabClicked(int index)
{
    if(isRibbonBarHideMode())
    {
        if(!m_d->stackedContainerWidget->isVisible())
...
/// \param index
///
void hnRibbonBar::onCurrentRibbonTabDoubleClicked(int index)
{
    Q_UNUSED(index);
...
void hnRibbonBar::onContextsCategoryPageAdded(hnRibbonCategory *category)
{
    Q_ASSERT_X(category != nullptr,"onContextsCategoryPageAdded","add nullptr page");
    m_d->stackedContainerWidget->addWidget(category);
}

void hnRibbonBar::updateRibbonElementGeometry()
{
    updateRibbonElementGeometry(m_d->ribbonStyle);
}

void hnRibbonBar::updateRibbonElementGeometry(hnRibbonBar::RibbonStyle style)
{
    switch(style)
    {
    case OfficeStyle:
...
hnRibbonButtonGroupWidget *hnRibbonBar::activeTabBarRightButtonGroup()
{
...
hnRibbonQuickAccessBar *hnRibbonBar::quickAccessBar()
{
    return m_d->quickAccessBar;
}

void hnRibbonBar::setRibbonStyle(hnRibbonBar::RibbonStyle v)
{
    m_d->ribbonStyle = v;
    m_d->lastShowStyle = v;
...
hnRibbonBar::RibbonStyle hnRibbonBar::currentRibbonStyle() const
{
    return m_d->ribbonStyle;
}

hnRibbonBar::RibbonMode hnRibbonBar::currentRibbonMode() const
{
    return m_d->currentRibbonMode;
}

int hnRibbonBar::unusableTitleRegion() const
{
    return m_d->unusableTitleRegion;
}

void hnRibbonBar::setUnusableTitleRegion(int v)
{
    m_d->unusableTitleRegion = v;
}

bool hnRibbonBar::eventFilter(QObject *obj, QEvent *e)
{
    if(obj)
    {
        //调整多文档时在窗口模式下的按钮更新
...
void hnRibbonBar::paintEvent(QPaintEvent *e)
{
    Q_UNUSED(e);
...
void hnRibbonBar::paintInNormalStyle()
{
    QPainter p(this);
    //!
    paintBackground(p);
...
void hnRibbonBar::paintInWpsLiteStyle()
{
    QPainter p(this);
    //!
    paintBackground(p);
...
void hnRibbonBar::paintContextCategoryTab(QPainter &painter, const QString &title, QRect contextRect, const QColor &color)
{
    //绘制上下文标签
    //首先有5像素的实体
...
void hnRibbonBar::resizeEvent(QResizeEvent *e)
{
    Q_UNUSED(e);
    switch(m_d->ribbonStyle)
    {
...
void hnRibbonBar::resizeInNormalStyle()
{
    ...
    if(m_d->applitionButton)
    {
        if(m_d->applitionButton->isVisible())
        {
           /* m_d->applitionButton->move(x,y);*/
			m_d->applitionButton->move(x, 0.0);
...
void hnRibbonBar::resizeInWpsLiteStyle()
{
...
void hnRibbonBar::paintBackground(QPainter &painter)
{
    painter.save();
    QPalette pl = palette();
...
/// x表示左边界，y表示右边界
///
void hnRibbonBar::paintWindowTitle(QPainter &painter, const QString &title, const QRect &titleRegion)
{
    painter.save();
...
void hnRibbonBar::paintWindowIcon(QPainter &painter, const QIcon &icon)
{
    painter.save();
    if(!icon.isNull())
    {
